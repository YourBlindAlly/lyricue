import type { Strings } from './en';

// Ukrainian translation, drafted 2026-09-05 for testers who use LyriCue in
// Ukrainian. Not reviewed by a native speaker yet — Rusty doesn't know a
// Ukrainian speaker himself, so this ships as a first draft and gets
// corrected from real tester feedback rather than a review round-trip
// beforehand. Keep this file's key order matching src/i18n/en.ts exactly so
// the two are easy to compare line-by-line.
export const uk: Strings = {
  about: {
    backButtonLabel: 'Назад',
    heading: 'Про LyriCue',
    introParagraph:
      'LyriCue читає ваші тексти пісень вголос, по одному рядку за раз, поки ви виступаєте наживо. Я створив цей додаток для себе. Я незрячий співак і автор пісень, і читати текст пісні на сцені для мене ніколи не було варіантом. LyriCue так само добре працює і для зрячих виконавців.',
    gettingStartedParagraph:
      'Початок роботи: найшвидший спосіб — спочатку спробувати одну з вбудованих демонстраційних пісень, щоб відчути, як це працює. А якщо є пісня, яку ви вже знаєте напам’ять, скористайтеся голосовим набором телефону, щоб продиктувати її прямо на екран «Нова пісня», кажучи «новий рядок» між рядками, щоб кожен опинився на своєму окремому рядку. Потім двічі торкніться кнопки «Завантажити пісню», щоб вивести пісню на екран тексту.',
    lyricsScreenParagraph:
      'Торкніться центру екрана, і VoiceOver скаже «Текст пісні». Перейдіть до наступного або попереднього рядка, провівши одним пальцем вгору чи вниз, або за допомогою ножної педалі.',
    voiceParagraph:
      'LyriCue має власний голос, окремий від VoiceOver, тому він звучить і працює однаково незалежно від того, увімкнено VoiceOver чи ні.',
    feedbackParagraph: (email: string) => `Щось не працює, або є ідея щодо нової функції? Напишіть на ${email}.`,
    websiteButtonLabel: 'Відвідати сайт',
    websiteButtonAccessibilityHint: 'Відкриває сайт LyriCue, включно з інструментами Lyric Editor та Setlist Builder.',
    websiteFailedTitle: 'Не вдалося відкрити сайт',
    websiteFailedMessage: (url: string) => `Відвідайте ${url} безпосередньо.`,
    toolsButtonLabel: 'Онлайн-інструменти',
    toolsButtonAccessibilityHint: 'Відкриває інструменти Lyric Editor та Setlist Builder напряму, минаючи загальний сайт.',
    toolsFailedTitle: 'Не вдалося відкрити онлайн-інструменти',
    toolsFailedMessage: (url: string) => `Відвідайте ${url} безпосередньо.`,
    contactButtonLabel: 'Зв\'язатися з розробником',
    contactButtonAccessibilityHint: 'Відкриває Пошту з новим листом, уже адресованим Расті.',
    contactFailedTitle: 'Не вдалося відкрити Пошту',
    contactFailedMessage: (email: string) => `Напишіть безпосередньо на ${email}.`,
    versionText: (version: string) => `Версія ${version}`,
    getStartedButtonLabel: 'Почати',
  },

  library: {
    heading: 'Ваші пісні',
    infoAccessibilityLabel: 'Інформація про LyriCue',
    infoLinkText: 'Інфо',
    dropboxButtonLabel: 'Dropbox',
    setlistsButtonLabel: 'Сетлисти',
    addSongButtonLabel: 'Додати пісню',
    searchButtonLabel: 'Пошук пісні',
    importingLabel: 'Імпортування…',
    importFileLabel: 'Імпортувати файл',
    sortButtonLabel: (mode: string) => `Сортування: ${mode}`,
    sortButtonHint: 'Проведіть вгору або вниз, щоб змінити, або торкніться, щоб перемкнути.',
    sortText: 'Сортування',
    nextSortActionLabel: 'Наступний порядок сортування',
    previousSortActionLabel: 'Попередній порядок сортування',
    filterButtonLabel: (mode: string) => `Фільтр: ${mode}`,
    filterButtonHint: 'Проведіть вгору або вниз, щоб змінити, або торкніться, щоб перемкнути.',
    filterText: 'Фільтр',
    nextFilterActionLabel: 'Наступний фільтр',
    previousFilterActionLabel: 'Попередній фільтр',
    filterModeLibraryLabel: 'Усі пісні',
    filterModeNoActiveSetlistLabel: 'Активний сетлист (немає)',
    filterModeNotebookLabel: 'Нотатник',
    noActiveSetlistFilterText: 'Немає активного сетлиста. Почніть його з екрана «Сетлисти», щоб скористатися цим фільтром.',
    emptySetlistFilterText: (name: string) => `Пісні з «${name}» ще не завантажені на цей пристрій.`,
    emptyNotebookText: 'У вашому нотатнику поки що нічого немає. Додайте пісню туди з Dropbox, щоб побачити її тут.',
    notebookRowAccessibilityHint: 'Двічі торкніться, щоб відкрити. Проведіть вгору або вниз, щоб перенести до бібліотеки.',
    promoteToLibraryActionLabel: 'Перенести до бібліотеки',
    promotedToLibraryAnnouncement: (name: string) => `Перенесено «${name}» до вашої бібліотеки`,
    emptyNotebookFileAlertTitle: 'Нема чого завантажувати',
    emptyNotebookFileAlertMessage: (name: string) => `У файлі «${name}» немає жодного рядка тексту.`,
    notebookOpenFailedAlertTitle: 'Не вдалося відкрити цю пісню',
    notebookPromoteFailedAlertTitle: 'Не вдалося перенести цю пісню',
    emptyLibraryText: 'Поки що немає пісень. Вставте текст, імпортуйте файл або підключіть Dropbox, щоб почати.',
    searchLabel: 'Пошук',
    searchPlaceholder: 'Пошук у вашій бібліотеці',
    searchAccessibilityLabel: 'Пошук у вашій бібліотеці за назвою або виконавцем',
    cancelSearchLabel: 'Скасувати',
    noSearchResultsText: (query: string) => `Пісень, що відповідають "${query}", не знайдено.`,
    songRowAccessibilityLabel: (title: string, source: string) => `${title}, ${source}`,
    songRowAccessibilityHint: 'Двічі торкніться, щоб відкрити. Проведіть вгору або вниз для інших дій.',
    editActionLabel: 'Редагувати',
    deleteActionLabel: 'Видалити',
    addToSetlistActionLabel: (setlistName: string) => `Додати до ${setlistName}`,
    addToNewSetlistActionLabel: 'Додати до нового сетлиста',
    addedToSetlistAnnouncement: (setlistName: string) => `Додано до ${setlistName}`,
    alreadyInSetlistAnnouncement: (setlistName: string) => `Вже є в ${setlistName}`,
    newSetlistPromptTitle: 'Назва нового сетлиста',
    newSetlistDefaultName: (date: string) => `Новий сетлист, ${date}`,
    createSetlistLabel: 'Створити',
    resumeSetlistLabel: (name: string, current: number, total: number) =>
      `Продовжити ${name}, пісня ${current} з ${total}`,
    resumeSetlistHint: 'Двічі торкніться, щоб продовжити. Проведіть вгору або вниз, щоб почати спочатку або припинити слідування.',
    startOverActionLabel: 'Почати спочатку',
    stopFollowingActionLabel: 'Припинити слідування',
    newSetlistCreatedAnnouncement: (setlistName: string) => `Створено новий сетлист, ${setlistName}`,
    sourceLabelPasted: 'Вставлено',
    sourceLabelImportedFile: 'Імпортований файл',
    sourceLabelDropbox: 'Dropbox',
    sourceLabelSearch: 'Пошук',
    sourceLabelDemoSong: 'Демо-пісня',
    removeSongAlertTitle: 'Видалити пісню',
    removeSongAlertMessage: (title: string) => `Видалити «${title}» з вашої бібліотеки?`,
    cancelLabel: 'Скасувати',
    removeLabel: 'Видалити',
    filePickerUnresponsiveTitle: 'Провідник файлів не відповідає',
    filePickerUnresponsiveMessage:
      'Провідник файлів не відповів вчасно — це може статися під час переходу в Google Drive. Спробуйте ще раз і скористайтеся локальними файлами або iCloud, або кнопкою Dropbox для хмарних файлів.',
    importFailedTitle: 'Помилка імпорту',
  },

  inputScreen: {
    editHeading: 'Редагувати пісню',
    addHeading: 'Додати пісню',
    cancelLabel: 'Скасувати',
    titleLabel: 'Назва (необов’язково)',
    lyricsLabel: 'Текст пісні',
    titlePlaceholder: 'Назва пісні',
    lyricsPlaceholder:
      'Вставте або продиктуйте текст пісні, по одному рядку, розділяючи рядки клавішею Enter. Позначайте розділи піснi як тире-тире приспів, або приспів у квадратних дужках, за бажанням.',
    saveChangesLabel: 'Зберегти зміни',
    loadSongLabel: 'Завантажити пісню',
  },

  search: {
    searchFailedTitle: 'Помилка пошуку',
    backButtonLabel: 'Назад',
    heading: 'Пошук',
    notConfiguredText:
      'Пошук ще не налаштовано. Для цього потрібен серверний бекенд, який ще не підключено до цієї збірки додатка.',
    queryLabel: 'Назва пісні або виконавець',
    queryPlaceholder: 'Назва пісні або виконавець',
    cancelEditingLabel: 'Скасувати',
    searchingLabel: 'Пошук…',
    searchLabel: 'Пошук',
    noResultsText: 'Нічого не знайдено.',
    resultHint: 'Двічі торкніться, щоб додати цю пісню до бібліотеки.',
    keySuffix: (key: string) => `тональність ${key}`,
    emptyResultAlertTitle: 'Нема чого завантажувати',
    emptyResultAlertMessage: (title: string) => `У файлі «${title}» немає жодного рядка тексту.`,
    loadFailedAlertTitle: 'Не вдалося завантажити пісню',
  },

  settingsScreen: {
    backButtonLabel: 'Назад',
    heading: 'Налаштування',
    pedalLinkLabel: 'Педаль і керування',
    voiceLinkLabel: 'Голос',
  },

  pedalSettings: {
    backButtonLabel: 'Назад',
    heading: 'Педаль і керування',
    pedalConnectedStatus: 'Педаль або клавіатура підключена',
    pedalNotConnectedStatus: 'Педаль не підключена',
    actionLabelNext: 'Наступний рядок',
    actionLabelPrevious: 'Попередній рядок',
    notSetLabel: 'Не налаштовано',
    waitingForPressText: 'Очікування натискання…',
    cancelLabel: 'Скасувати',
    assignButtonLabel: 'Натисніть кнопку, щоб призначити',
    alertOnDisconnectLabel: 'Сповіщати про відключення педалі',
    alertOnDisconnectHint: 'Проведіть вгору, щоб увімкнути, вниз — щоб вимкнути.',
    alertOnDisconnectOnActionLabel: 'Увімкнути',
    alertOnDisconnectOffActionLabel: 'Вимкнути',
    alertOnDisconnectStateOnLabel: 'Увімкнено',
    alertOnDisconnectStateOffLabel: 'Вимкнено',
    repeatFeatureLabel: 'Повторювати рядок перед переходом далі',
    repeatFeatureHint:
      'Увімкнено за замовчуванням. Натискання "назад" повторює поточний рядок, перш ніж справді перейти назад; швидке "вперед" після цього також повторює замість переходу далі. Проведіть вгору, щоб увімкнути, вниз — щоб вимкнути.',
    repeatOnActionLabel: 'Увімкнути',
    repeatOffActionLabel: 'Вимкнути',
    repeatStateOnLabel: 'Увімкнено',
    repeatStateOffLabel: 'Вимкнено',
  },

  voiceSettings: {
    backButtonLabel: 'Назад',
    heading: 'Голос',
    reduceChatterLabel: 'Зменшити повідомлення VoiceOver під час виступу',
    reduceChatterHint:
      'Експериментально. Повідомляє VoiceOver, що на екрані тексту пісні активний аудіосеанс, тому він рідше перериває мовлення LyriCue — дотики та кнопки, як і раніше, працюють нормально. Проведіть вгору, щоб увімкнути, вниз — щоб вимкнути.',
    toggleOnActionLabel: 'Увімкнути',
    toggleOffActionLabel: 'Вимкнути',
    stateOnLabel: 'Увімкнено',
    stateOffLabel: 'Вимкнено',
    speakingSpeedHint: 'Проведіть вгору, щоб швидше, вниз — щоб повільніше.',
    fasterActionLabel: 'Швидше',
    slowerActionLabel: 'Повільніше',
    speakingSpeedText: 'Швидкість мовлення',
    speakingVolumeHint: 'Проведіть вгору, щоб гучніше, вниз — щоб тихіше.',
    louderActionLabel: 'Гучніше',
    quieterActionLabel: 'Тихіше',
    speakingVolumeText: 'Гучність мовлення',
    systemDefaultLabel: 'Системний за замовчуванням',
    selectedLabel: 'Вибрано',
    currentVoiceLabel: 'Поточний голос',
    previewButtonLabel: 'Прослухати',
    showLowQualityLabel: 'Показати голоси нижчої якості',
    showLowQualityHint:
      'Вимкнено за замовчуванням — показує лише голоси вищої якості (Enhanced або Premium) для кожної мови, а не стандартної якості. Проведіть вгору, щоб увімкнути, вниз — щоб вимкнути.',
    tickSoundLabel: 'Відтворювати гудок при переході між рядками',
    tickSoundHint: 'Увімкнено за замовчуванням. Вібрація при переході залишається увімкненою в будь-якому разі. Проведіть вгору, щоб увімкнути, вниз — щоб вимкнути.',
    languageDetectionLabel: 'Визначення мови',
    languageDetectionHint:
      'За замовчуванням — евристика. Визначає, як вгадується мова пісні для вибору відповідного голосу. Проведіть вгору для Apple, вниз для евристики.',
    languageDetectionAppleLabel: 'Apple',
    languageDetectionHeuristicLabel: 'Евристика',
    languageDetectionSwitchToAppleActionLabel: 'Перемкнути на Apple',
    languageDetectionSwitchToHeuristicActionLabel: 'Перемкнути на евристику',
    languageVoicesLinkLabel: 'Голоси для мов',
    languageVoicesLinkHint: 'Налаштуйте конкретний голос для окремих мов.',
    reduceHintsLabel: 'Зменшити підказки для елементів керування',
    reduceHintsHint:
      'Вимкнено за замовчуванням. Коли увімкнено, VoiceOver пропускає підказку щодо використання кожного елемента керування ("проведіть вгору, щоб пришвидшити") і просто озвучує його назву та значення — корисно, коли ви вже знаєте, як працює кожен елемент керування. Проведіть вгору, щоб увімкнути, вниз — щоб вимкнути.',
    loadingVoicesText: 'Завантаження голосів…',
    noVoicesFoundText: 'На цьому пристрої не знайдено голосів.',
    voiceRowAccessibilityLabel: (name: string, language: string) => `${name}, ${language}`,
    previewSpokenText: 'Ось як звучатиме ваш текст пісні.',
  },

  languageVoices: {
    backButtonLabel: 'Назад',
    heading: 'Голоси для мов',
    introText:
      'Оберіть конкретний голос для мови, щоб пісні, визначені або позначені цією мовою, завжди використовували його — замість того, щоб LyriCue вгадувала, який встановлений голос обрати. Більшості людей потрібно лише одна-дві мови; ви можете додати скільки завгодно.',
    emptyText: 'Голоси для мов ще не налаштовано — для будь-якої визначеної мови автоматично обирається підходящий встановлений голос.',
    entryAccessibilityLabel: (language: string, voice: string) => `${language}: ${voice}. Двічі торкніться, щоб змінити.`,
    entryHint: 'Двічі торкніться, щоб змінити голос для цієї мови.',
    removeButtonLabel: (language: string) => `Видалити ${language}`,
    removeButtonLabel2: 'Видалити',
    addHeading: 'Додати мову',
    addLanguageText: 'Мова',
    addLanguageHint: 'Проведіть вгору або вниз, щоб змінити мову, для якої додається голос.',
    nextLanguageActionLabel: 'Наступна мова',
    previousLanguageActionLabel: 'Попередня мова',
    chooseVoiceButtonLabel: (language: string) => `Обрати голос для ${language}`,
    pickerHeading: (language: string) => `Голос для ${language}`,
  },

  dropboxBrowse: {
    backButtonLabel: 'Назад',
    heading: 'Dropbox',
    notConfiguredInfoText1:
      'Dropbox ще не налаштовано. Щоб увімкнути його, створіть додаток на dropbox.com/developers/apps, додайте дозволи files.metadata.read і files.content.read, додайте цей redirect URI в розділі OAuth 2:',
    notConfiguredInfoText2: 'Потім дайте мені ключ додатка, і я його підключу.',
    connectDropboxLabel: 'Підключити Dropbox',
    sortLabel: (mode: string) => `Сортування: ${mode}`,
    sortHint: 'Проведіть вгору або вниз, щоб змінити, або торкніться, щоб перемкнути.',
    sortText: 'Сортування',
    nextSortActionLabel: 'Наступний порядок сортування',
    previousSortActionLabel: 'Попередній порядок сортування',
    selectLabel: 'Вибрати',
    selectModeCancelLabel: 'Скасувати',
    selectHint: 'Виберіть кілька пісень для імпорту.',
    disconnectLabel: 'Відключити',
    connectedAsText: (email: string) => `Підключено як ${email}`,
    deselectAllLabel: 'Скасувати вибір усіх',
    selectAllLabel: 'Вибрати всі',
    importSelectedLabel: (count: number) => `Імпортувати вибрані (${count})`,
    importingProgressText: (done: number, total: number) => `Імпортування ${done} з ${total}…`,
    emptyText: 'Тут нічого немає.',
    entryFolderAccessibilityLabel: (name: string) => `${name}, папка`,
    entrySelectedAccessibilityLabel: (name: string) => `${name}, вибрано`,
    entryNotSelectedAccessibilityLabel: (name: string) => `${name}, не вибрано`,
    entrySongAccessibilityLabel: (name: string) => `${name}, пісня`,
    couldntConnectAlertTitle: 'Не вдалося підключитися до Dropbox',
    emptyFileAlertTitle: 'Порожній файл',
    emptyFileAlertMessage: (name: string) => `У файлі «${name}» немає жодного рядка тексту.`,
    downloadFailedAlertTitle: 'Помилка завантаження',
    importCompleteAlertTitle: 'Імпорт завершено',
    // Numeral + colon phrasing sidesteps Ukrainian's three-way count
    // agreement (пісня/пісні/пісень) rather than getting it wrong — a
    // common, acceptable pattern in localized UI copy.
    importedSongsText: (count: number) => `Імпортовано пісень: ${count}.`,
    skippedSongsText: (count: number) => `Пропущено без тексту: ${count}.`,
    failedSongsText: (count: number) => `Не вдалося завантажити: ${count}.`,
  },

  setlists: {
    backButtonLabel: 'Назад',
    heading: 'Сетлисти',
    newSetlistLabel: 'Новий сетлист',
    nothingToPlayAlertTitle: 'Нема чого відтворювати',
    nothingToPlayAlertMessage: (name: string) =>
      `Жодної пісні із «${name}» не знайдено у вашій бібліотеці. Спочатку імпортуйте їх, а потім спробуйте цей сетлист знову.`,
    couldntLoadSetlistAlertTitle: 'Не вдалося завантажити сетлист',
    deleteSetlistAlertTitle: 'Видалити сетлист',
    deleteSetlistAlertMessage: (name: string) => `Видалити «${name}»? Цю дію не можна скасувати в додатку.`,
    cancelLabel: 'Скасувати',
    deleteLabel: 'Видалити',
    deleteFailedAlertTitle: 'Помилка видалення',
    emptyText: 'Сетлистів поки немає. Натисніть «Новий сетлист», щоб створити перший.',
    activePlayingAccessibilityLabel: (name: string, current: number, total: number) =>
      `${name}, Відтворюється — пісня ${current} з ${total}`,
    resumeHint: 'Двічі торкніться, щоб продовжити. Проведіть вгору або вниз, щоб почати спочатку, редагувати або припинити слідування.',
    playHint: 'Двічі торкніться, щоб відтворити. Проведіть вгору або вниз для редагування або видалення.',
    stopFollowingActionLabel: 'Припинити слідування',
    startOverActionLabel: 'Почати спочатку',
    refreshActionLabel: 'Оновити пісні',
    refreshDoneAlertTitle: 'Пісні оновлено',
    refreshDoneAlertMessage: (refreshed: number, skipped: string[], failed: string[]) => {
      const parts = [`Оновлено з Dropbox: ${refreshed}.`];
      if (skipped.length > 0) parts.push(`Не в Dropbox, залишено як є: ${skipped.join(', ')}.`);
      if (failed.length > 0) parts.push(`Не вдалося завантажити: ${failed.join(', ')}.`);
      return parts.join(' ');
    },
    editActionLabel: 'Редагувати',
    playingStatusText: (current: number, total: number) => `Відтворюється — пісня ${current} з ${total}`,
    importFromDropboxLabel: 'Імпортувати з Dropbox',
  },

  importSetlist: {
    backButtonLabel: 'Назад',
    heading: 'Імпорт сетлиста',
    explainerText:
      'Це файли CSV сетлистів у вашій папці /setlists на Dropbox — включно з будь-якими, створеними іншим інструментом, наприклад веб-конструктором сетлистів. Торкніться, щоб додати його до бібліотеки тут.',
    notConfiguredInfoText1:
      'Dropbox ще не налаштовано. Щоб увімкнути його, створіть додаток на dropbox.com/developers/apps, додайте дозволи files.metadata.read і files.content.read, додайте цей redirect URI в розділі OAuth 2:',
    notConfiguredInfoText2: 'Потім дайте мені ключ додатка, і я його підключу.',
    connectDropboxLabel: 'Підключити Dropbox',
    emptyText: 'У папці /setlists на Dropbox не знайдено файлів сетлистів.',
    entryAccessibilityLabel: (name: string) => `${name}, сетлист`,
    entryHint: 'Двічі торкніться, щоб імпортувати.',
    couldntConnectAlertTitle: 'Не вдалося підключитися до Dropbox',
    importedAlertTitle: 'Сетлист імпортовано',
    importedAlertMessage: (name: string, count: number) =>
      `«${name}» тепер у вашому списку сетлистів. Пісень: ${count}.`,
    importFailedAlertTitle: 'Помилка імпорту',
  },

  setlistCreator: {
    backButtonLabel: 'Назад',
    heading: (isEditing: boolean) => (isEditing ? 'Редагувати сетлист' : 'Новий сетлист'),
    setlistNamePlaceholder: 'Назва сетлиста',
    songsInSetlistHeading: (count: number) => `Пісні в цьому сетлисті (${count})`,
    nothingAddedText: 'Ще нічого не додано — проведіть по пісні у вашій бібліотеці, щоб додати її, або скористайтеся пошуком нижче.',
    moveUpActionLabel: 'Перемістити вгору',
    moveDownActionLabel: 'Перемістити вниз',
    removeActionLabel: 'Видалити',
    entryAccessibilityLabel: (position: number, title: string) => `${position}. ${title}`,
    entryHint: 'Проведіть вгору або вниз для дій переміщення та видалення.',
    savingLabel: 'Збереження…',
    saveSetlistLabel: (isEditing: boolean) => (isEditing ? 'Зберегти зміни' : 'Зберегти сетлист'),
    searchPlaceholder: 'Пошук у ваших піснях',
    noSongsMatchText: 'Немає відповідних пісень.',
    addFromLibraryInstructionText:
      'Щоб додати пісню, проведіть по ній у вашій бібліотеці та виберіть «Додати до сетлиста» — або скористайтеся пошуком тут.',
    addedAccessibilityLabel: (title: string) => `${title}, Додано`,
    addHint: 'Двічі торкніться, щоб додати.',
    addedMarkText: 'Додано',
    nameNeededAlertTitle: 'Потрібна назва',
    nameNeededAlertMessage: 'Дайте цьому сетлисту назву перед збереженням.',
    noSongsAddedAlertTitle: 'Не додано жодної пісні',
    noSongsAddedAlertMessage: 'Додайте хоча б одну пісню перед збереженням.',
    saveFailedAlertTitle: 'Помилка збереження',
    couldntLoadForEditingAlertTitle: 'Не вдалося завантажити сетлист',
  },

  promptScreen: {
    keyAndCapoSuffix: (key?: string, capo?: string) => {
      const parts: string[] = [];
      if (key) parts.push(`Тональність ${key}`);
      if (capo) parts.push(`Капо ${capo}`);
      return parts.length > 0 ? ` — ${parts.join(', ')}` : '';
    },
    fullSetlistText: (name: string, current: number, total: number) => `${name} — пісня ${current} з ${total}`,
    randomProgressText: (name: string, played: number, total: number) =>
      `${name} — зіграно ${played} з ${total} пісень`,
    pedalConnectedStatusLabel: 'Педаль: Підключена',
    pedalNotConnectedStatusLabel: 'Педаль: Не підключена',
    voiceLinkLabel: 'Голос',
    linesLabel: (preset: string) => `Рядки: ${preset}`,
    linesText: 'Рядки',
    linesHint: 'Проведіть вгору для довших рядків, вниз — для коротших.',
    longerActionLabel: 'Довше',
    shorterActionLabel: 'Коротше',
    chordsOnLabel: 'Акорди: Увімк.',
    chordsOffLabel: 'Акорди: Вимк.',
    chordsText: 'Акорди',
    chordsHint: 'Проведіть вгору, щоб увімкнути, вниз — щоб вимкнути.',
    chordsOnActionLabel: 'Увімк.',
    chordsOffActionLabel: 'Вимк.',
    lineBreaksAtChordsLabel: 'Розриви рядків: Акорди',
    lineBreaksAtWordsLabel: 'Розриви рядків: Слова',
    lineBreaksText: 'Розриви рядків',
    lineBreaksHint: 'Проведіть вгору для розриву за акордами, вниз — за словами.',
    lineBreaksChordsValue: 'Акорди',
    lineBreaksWordsValue: 'Слова',
    chordPitchOnLabel: 'Тон акордів: Вищий',
    chordPitchOffLabel: 'Тон акордів: Звичайний',
    chordPitchText: 'Тон акордів',
    chordPitchHint: 'Проведіть вгору для вищого тону, вниз для звичайного.',
    chordPitchOnActionLabel: 'Вищий',
    chordPitchOffActionLabel: 'Звичайний',
    editLinkLabel: 'Редагувати',
    settingsLinkLabel: 'Налаштування',
    libraryLinkLabel: 'Бібліотека',
    lyricsAreaAccessibilityLabel: 'Текст пісні',
    lyricsAreaHint: 'Проведіть вниз для наступного рядка, вгору — для попереднього.',
    endOfSongLabel: 'Кінець пісні',
    previousSongAccessibilityLabel: 'Попередня пісня',
    previousSongButtonText: '‹‹ Пісня',
    previousLineAccessibilityLabel: 'Попередній рядок',
    previousLineButtonText: '‹ Попередній',
    nextLineAccessibilityLabel: 'Наступний рядок',
    nextLineButtonText: 'Наступний ›',
    nextSongAccessibilityLabel: 'Наступна пісня',
    nextSongButtonText: 'Пісня ››',
    setlistJumpHint: 'Проведіть вниз для наступної пісні, вгору — для попередньої.',
    noMoreSongsInSetlistAnnouncement: 'Більше немає пісень у цьому сетлисті.',
    alreadyAtFirstSongAnnouncement: 'Це вже перша пісня.',
    randomSetlistLabel: 'Випадкова наступна пісня',
    randomSetlistHint: 'Проведіть вгору, щоб увімкнути, вниз — щоб вимкнути. Коли увімкнено, наступна пісня обирається випадково, без повторів, поки не прозвучать усі пісні сетліста.',
    randomSetlistOnActionLabel: 'Увімкнути',
    randomSetlistOffActionLabel: 'Вимкнути',
    randomSetlistStateOnLabel: 'Увімкнено',
    randomSetlistStateOffLabel: 'Вимкнено',
    pedalDisconnectedAnnouncement: 'Педаль відключена. Використовуються кнопки на екрані.',
    pedalConnectedAnnouncement: 'Педаль підключена.',
  },

  app: {},
};
